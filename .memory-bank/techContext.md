...
- **Nota**: El backoffice fue migrado a Next.js 16 App Router, incluyendo el Gestor Centralizado de Incidencias como ruta protegida `/incidents-manager`. Las páginas HTML legacy servidas por FastAPI han sido eliminadas; FastAPI ahora funciona exclusivamente como API REST en puerto 8000 y Next.js sirve la interfaz en puerto 3000.
...
- **Frontend**: FastAPI ya NO sirve páginas HTML. La interfaz es servida exclusivamente por Next.js 16 (puerto 3000) en `uis/backoffice/`. FastAPI funciona únicamente como API REST.
...
- **Frontend**: Next.js 16 App Router (portfolio, login, registro, perfil, proveedores, analizador CSV, gestor de incidencias). FastAPI solo provee API REST.
...
│   ├── useDashboard.ts
│   ├── useSuppliers.ts
│   ├── useIncidentAnalyzer.ts
│   └── useIncidentManager.ts
├── services/
│   └── api.ts              # Cliente HTTP con funciones CRUD y auth
├── lib/
│   ├── constants.ts        # API_BASE="/api", categorías de proveedores
│   └── incidents.ts        # Constantes UI del gestor (etiquetas, estilos, transiciones)
└── public/
