#!/usr/bin/env python3
"""
seed.py — Seeder del Directorio de Proveedores (TrackFlow).

Carga los proveedores iniciales definidos en SUPPLIERS_SEED en TinyDB.
Se ejecuta con:  uv run seed  (o: python -m seed)

Comportamiento:
- Verifica que la tabla de proveedores esté vacía antes de insertar.
- Si ya hay datos, muestra un aviso y no duplica.
- Confirma en consola cuántos registros se insertaron.
"""
...
def seed():
    """Ejecuta la carga de datos iniciales si la tabla está vacía."""
    existing_count = len(suppliers_table)
    ...
    print(f"✅ Se insertaron {inserted} proveedores en la base de datos.")
    print(f"   Base de datos: suppliers_db.json")
