...
export function TransformationsPanel({
  ...
  const [summaryLastKey, setSummaryLastKey] = useState<string>("countCat");
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-[#c89d66] bg-[#f8fbff] p-4">
          ...
          <div className="mb-2 flex flex-wrap gap-2">
            ...
            <button onClick={() => showResult("score", onScoreCarrier(clamp(carrierIdx, carriers.length), clamp(shipmentIdx, shipments.length), clamp(productIdx, products.length)))} className="rounded-lg bg-[#14263a] px-4 py-1 text-xs font-medium text-[#f8fbff] transition-colors hover:bg-[#1d4f7a]">{t("dashboard.transform.score_action")}</button>
          </div>
        ...
        <div className="rounded-lg border border-[#c89d66] bg-[#f8fbff] p-4">
          ...
          <div className="mb-2 flex gap-2">
            <label className="text-xs text-[#2f4a62]">{t("dashboard.transform.shipment")} <input type="number" min="0" max={shipments.length - 1} value={shipmentIdx} onChange={(e) => setShipmentIdx(e.target.value)} className="w-12 rounded border border-[#c89d66] bg-[#f3ddba] px-1 py-0.5 text-xs" /></label>
            <button onClick={() => showResult("best", onSelectBest(clamp(shipmentIdx, shipments.length), clamp(productIdx, products.length)))} className="rounded-lg bg-[#14263a] px-4 py-1 text-xs font-medium text-[#f8fbff] transition-colors hover:bg-[#1d4f7a]">{t("dashboard.transform.select_action")}</button>
          </div>
        ...
        <div className="rounded-lg border border-[#c89d66] bg-[#f8fbff] p-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => { showResult("countCat", onCountByCategory()); setSummaryLastKey("countCat"); }} className="rounded-lg bg-[#14263a] px-4 py-1 text-xs font-medium text-[#f8fbff] transition-colors hover:bg-[#1d4f7a]">{t("dashboard.transform.count_category")}</button>
            <button onClick={() => { showResult("invVal", onInventoryValue()); setSummaryLastKey("invVal"); }} className="rounded-lg bg-[#14263a] px-4 py-1 text-xs font-medium text-[#f8fbff] transition-colors hover:bg-[#1d4f7a]">{t("dashboard.transform.inventory_value")}</button>
            <button onClick={() => { showResult("avgDist", onAvgDistance()); setSummaryLastKey("avgDist"); }} className="rounded-lg bg-[#14263a] px-4 py-1 text-xs font-medium text-[#f8fbff] transition-colors hover:bg-[#1d4f7a]">{t("dashboard.transform.avg_distance")}</button>
          </div>
        ...
        <div className="rounded-lg border border-[#c89d66] bg-[#f8fbff] p-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => showResult("groupStatus", onGroupByStatus())} className="rounded-lg bg-[#14263a] px-4 py-1 text-xs font-medium text-[#f8fbff] transition-colors hover:bg-[#1d4f7a]">{t("dashboard.transform.by_status")}</button>
            <div className="flex gap-1">
              ...
              <button onClick={() => showResult("topC", onTopCarriers(parseInt(topN) || 3))} className="rounded-lg bg-[#14263a] px-4 py-1 text-xs font-medium text-[#f8fbff] transition-colors hover:bg-[#1d4f7a]">{t("dashboard.transform.top_carriers")}</button>
            </div>
          </div>
